...
+ (BOOL)addSkipBackupAttributeToItemAtURL:(NSURL *)URL
{
    ...
    if(!success)
    {
        DebugLogError(@"Error excluding %@ from backup %@", [URL lastPathComponent], error);
    }
    return success;
...
+ (NSString *)getAppDocumentPath
{
    ...
    dispatch_once(&onceToken, ^{
        NSString *appKey = @"2663bf1d5cea0701a6e9375a2cc571aa";
...
+ (NSString *)userDirectory
{
    ...
    if ([userID length] == 0)
    {
        DebugLogError(@"Error: Get User Directory While UserID Is Empty");
    }
